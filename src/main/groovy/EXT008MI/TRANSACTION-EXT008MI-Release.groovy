 ...
 * 20221227   E. Dacillo                      CR for ticket 16604239
 */
...
public class Release extends ExtendM3Transaction {
  ...
  private int newOQPLSX = 0;
  private boolean hasMultiplePLSX = false;
  ...
  public void main() {
    ...
    //Check number of PLSX
    List<DBContainer> lstMHPICH1 = readMHPICH();
    if (lstMHPICH1.size() > 1) {
      hasMultiplePLSX = true;
    }
    logger.debug("lstMHPICH1=${lstMHPICH1}/hasMultiplePLSX=${hasMultiplePLSX}");


    if (hasMultiplePLSX) {
      //Loop PLSX
      int tmpPLSX = Integer.parseInt(inPLSX);
      String tmpError = "";
      for (DBContainer recMHPICH : lstMHPICH1) {
        int PIPLSX = recMHPICH.getInt("PIPLSX");
        logger.debug("hasMultiplePLSX PIPLSX=${PIPLSX}/tmpPLSX=${tmpPLSX}/${(PIPLSX >= tmpPLSX)}/");
        inPLSX = PIPLSX.toString();
        logger.debug("hasMultiplePLSX PIPLSX=${PIPLSX}/tmpPLSX=${tmpPLSX}/${(PIPLSX >= tmpPLSX)}/inPLSX=${inPLSX}/");
        ...
        Closure<?> updPickHeadHandler = { Map<String, String> response ->
          apiResp = response;
          if (response.error) {
            tmpError = response.errorMessage;
          }
        }
      ...
      logger.debug("not MultiplePLSX = ${hasMultiplePLSX}/");
      // - Execute MWS420MI.UpdPickHead
      def updPickHeadParams = ["DLIX": inDLIX, "PLSX": inPLSX, "TEAM": inTEAM, "PICK": inPICK, "SEEQ": inSEEQ, "DEV": inZDEV, "DOVA": inDOVA];
      Closure<?> updPickHeadHandler = { Map<String, String> response ->
        apiResp = response;
   ...
   */
  void packageBasedPickingPerformSplit() {
    //=========================================
    //MWMNGPCS - RUPD
   ...
   * @return
   */
  def void rtvCapacityParam(DBContainer recMHPICH) {
  ...
  def void spltNoOfPac(DBContainer recMHPICH) {
    ...
    for (DBContainer recMITALO30: listMITALO30) {
      ...
      //Retrieve latest MHPICH
      List<DBContainer> lstMHPICH = readCurrentMHPICH(recMHPICH);
   ...
   * @return
   */
  def void processMFTRND(DBContainer recMFTRND, long savePLRN, long newPLRN) {
    // Change MITALO references by deleting the MFTRND record with reporting nunber from the current MITALO record
   ...
   * @return
   */
  def void delAllocationLine(DBContainer recMITALO) {
   ...
   * @return
   */
  def void delMHPICH(DBContainer recMHPICH, int orgITALOPLSX) {
   ...
   * @return
   */
  def void updLastMHPICH(DBContainer recMHPICH) {
    Closure<?> updateMHPICH = { LockedResult lockedResult ->
   ...
   */
  void calculatePackageBasedSplit(DBContainer recMHPICH) {
    // Level breaks
    breakOnLine = false;
    ...
    for (DBContainer recMFTRND : lstMFTRND) {
      ...
      queryMITALO.readAll(containerMITALO, 2, { DBContainer record ->
        //Retrieve latest MHPICH
        List<DBContainer> lstMHPICH = readCurrentMHPICHByPLSX();
        for (DBContainer recCurMHPICH: lstMHPICH) {
          recMHPICH = recCurMHPICH;
        ...
        if (containerMITALO.getInt("MQPLSX") == XSPLSX) {
          ...
          if (checkPickLines &&
            ...
            if (lineUpdate) {
              ...
              //read MITALO50 record again with the new PLSX to refresh record
   ...
   * @return
   */
  def void updSubLines(DBContainer recMITALO, DBContainer recMHPICH) {
   ...
   * @return
   */
  def void updMFTRND_PLRN(DBContainer recMFTRND, long PLRN) {
   ...
   * @return
   */
  def void updOutstandingQty(DBContainer recMITALO) {
    Closure<?> updateMITALO = { LockedResult lockedResult ->
  ...
  def void splitLineMHPICHMITALO(DBContainer recMHPICH, DBContainer recMITALO) {
    ...
    if (packageCheck || breakOnLine) {
      if (!fromspltNoOfPac) {
        ...
        DBContainer container = query.getContainer();
        container.set("PICONO", XXCONO.toInteger());
        container.set("PIDLIX", inDLIX.toLong());
        container.set("PIPLSX", newOQPLSX);
        if (query.read(container)) {
          logger.debug("splitLineMHPICHMITALO: call updateAllocations MHPICH current by reverse record=${container}/should be the latest PLSX");
          updateAllocations(recMITALO, container);
        }
      }
    }
    // Save variables
  ...
  def void updatePickingListStatus(DBContainer recMHPICH) {
    ...
    if (packageCheck ||
      ...
      Closure<?> updMHDISH = { LockedResult lockedResult ->
        newOQPLSX = lockedResult.getInt("OQPLSX") + 1;
        logger.debug("updatePickingListStatus:update OQPLSX newOQPLSX=${newOQPLSX}");
        lockedResult.set("OQPLSX", newOQPLSX);
   ...
   */
  void updatePickingListHeader(DBContainer recMHPICH) {
...
//          This will update the current pick list
    ...
    logger.debug("updatePickingListHeader.current OQPLSX=${newOQPLSX}/packageCheck=${packageCheck}/PIPLSX=${recMHPICH.getInt("PIPLSX")}/");;
    ...
    recMHPICH.set("PIDLIX", recMHPICH.getLong("PIDLIX"));
    recMHPICH.set("PIPLSX", recMHPICH.getInt("PIPLSX"));
    ...
    if (packageCheck) {
      ...
      if (TOTCNT < XXNPLL ||
        TOTCNT == XXNPLL &&
        LINCNT > 1) {
        logger.debug("updatePickingListHeader: create newMHPICH with OQPLSX=${newOQPLSX}/recMHPICHPLSX=${recMHPICH.getLong("PIDLIX")}/");
        container.set("PICONO", XXCONO.toInteger());
        container.set("PIDLIX", recMHPICH.getLong("PIDLIX"));
        container.set("PIPLSX", newOQPLSX);
        container.set("PINPLL", 1);
   ...
   */
  void updateAllocations(DBContainer recMITALO, DBContainer recMHPICH) {
    ...
    //This should delete and insert the line with the correct PLSX
    Closure<?> updateMITALO = { LockedResult lockedResult ->
      ...
      if (breakOnLine || PKDLQT == 0) {
        ...
        //Delete current MITALO record with the old PLSX since a new MITALO record is added
        ...
        containerMITALO.set("MQRIDI", lockedResult.get("MQRIDI"));
        if (LINCNT > 1) {
          containerMITALO.set("MQPLSX", newOQPLSX);
        } else {
   ...
   */
  void calculatePickingTime(DBContainer recMITALO, double O0DLQT) {
  ...
  def void calculateAllocationBasedSplit(DBContainer recMHPICH) {
    ...
    newOQPLSX = recMHPICH.getInt("PIPLSX");
    for (DBContainer recMITALO30: lstMITALO30) {
      ...
      logger.debug("inside for loop calculateAllocationBasedSplit: newOQPLSX=${newOQPLSX}");
      //Retrieve latest MHPICH
      List<DBContainer> lstMHPICH = readCurrentMHPICHByPLSX();
      for (DBContainer recCurMHPICH: lstMHPICH) {
        recMHPICH = recCurMHPICH;
      ...
      if (checkPickLines &&
        ...
        if (lineUpdate) {
          ...
          //read MITALO30 record again with the new PLSX to refresh record
   ...
   * @return
   */
  def void updLastMHPICH_CASPLT(DBContainer recMHPICH) {
  ...
  /**
   * readMHPICH - read records in MHPICH
   * @return
   */
  List <DBContainer> readMHPICH() {
    List<DBContainer> containers = new ArrayList();
    ExpressionFactory expression = database.getExpressionFactory("MHPICH")
    expression = expression.eq("PIPISS", "30");
    expression = expression.and(expression.ge("PIPLSX", inPLSX));

    DBAction query = database.table("MHPICH")
      .index("00")
      .selection("PIDLIX", "PIPLSX", "PIPISS")
      .build();
    DBContainer container = query.getContainer();
    container.set("PICONO", XXCONO.toInteger());
    container.set("PIDLIX", inDLIX.toLong());
    query.readAll(container, 2, { DBContainer record ->
      containers.add(record.createCopy());
    });
    return containers;
  }
  ...
  /**
   * readCurrentMHPICHByPLSX - read record in MHPICH using new PLSX
   * @return
   */
  List <DBContainer> readCurrentMHPICHByPLSX() {
    List<DBContainer> containers = new ArrayList();
    DBAction query = database.table("MHPICH")
      .index("00")
      .selectAllFields()
    ...
    container.set("PIPLSX", newOQPLSX);
    if (query.read(container)) {
      containers.add(container.createCopy());
    }
    return containers;
  }
}
